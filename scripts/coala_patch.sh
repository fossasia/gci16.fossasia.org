...
if [ "$TRAVIS_BRANCH" = "gh-pages" ]; then
    ...
    if [ "$?" = 0 ]; then
        ...
        curl -X POST -H "Authorization: token $GITHUB_TOKEN" --data '{"title":"coala-fixes for build $TRAVIS_BUILD_NUMBER", "body":Automated Coala fixes: $TRAVIS_BUILD_NUMBER | Generated by Travis CI", "head":"$FIX_BRANCH", "base":"gh-pages"}' https://api.github.com/repos/fossasia/gci16.fossasia.org/pulls;
    else
